...
omiq_mtx = read.csv2(check.names = F,file = file.path(saveDir,"Original_Panel(31color)_SigMtx_fromOMIQ.csv"),sep = ",")
rownames(omiq_mtx) = omiq_mtx$USERM_id
...
#original 31 fluors panel
prefix = "Original_Panel(31color)_nochange"
fluors_selected = c(Sig_info$id[c(32:38,41:42,44,46:48,50:54,58,59,60,61,62,8,9,12,14,18,24,25,26,63)])
...
#replace with sig mtx from omiq
A = UsermObj[["A"]]
for (i in 1:nrow(omiq_mtx)) {
  A[,rownames(omiq_mtx)[i]] = as.numeric(omiq_mtx[rownames(omiq_mtx)[i],rownames(A)]) / 100
}
UsermObj[["A"]] = A
